...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📖 实战泛读速记", "🕹️ 考前通关测试", "🎵 歌剧歌词自由泛读", "🗂️ 核心单词总表", "📚 错题闯关"])
...
with tab5:
    ...
    # 初始化闯关状态
    if "is_wrong_quiz" not in st.session_state:
    ...
    if not st.session_state.wrong_book:
        ...
        if not st.session_state.is_wrong_quiz:
            if st.button("🚀 开启错题专项闯关", use_container_width=True):
                st.session_state.is_wrong_quiz = True
                st.rerun()
            st.write(f"当前共有 {len(st.session_state.wrong_book)} 道错题待复习。")
        else:
            # 随机抽题
            ...
            ans = st.text_input("请输入释义:", key="w_quiz_input")
